...
async function Connection(update, This, clearState) {
  ...
  if (connection === "connecting") {
    ...
    This.log("connect", "Author : AsifShinzo");
    This.log(
      "connect",
      `Name    : ${This?.user?.name !== undefined ? This?.user?.name : "Roronoa"}`,
    );
    This.log("connect", `Number  : ${This?.user?.id?.split(":")[0]}`);
    This.log("connect", `Version : v5.1.1`);
    This.log(
      "connect",
      `Time : ${new Date().toLocaleString("id", { timeZone: "Asia/Kolkata" })}`,
    );
    This.log("info", "Roronoa is connected!");
    if (This.commands) {
