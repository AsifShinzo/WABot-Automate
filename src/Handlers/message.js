        ...
        if (gc_link) {
          ...
          if (!M?.isAdmin && !M?.isMod) {
            if (!M?.isBotAdmin) {
              await Neko.sendTextMessage(
                M.from,
                "This Group has Antilink enabled. Bot requires Admin Access for it to work properly.",
              );
              return true;
            }
            await Neko.sendTextMessage(
      ...
      if (
        ...
        if (
          ...
          await Neko.sendMentionMessage(
            from,
            `*_This is a warning! @${sender.split("@")[0]} if you send nsfw again! You will get kicked out of this group_*`,
            [sender],
          );
          return true;
        }
